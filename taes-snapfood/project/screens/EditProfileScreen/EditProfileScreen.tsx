...
import { clearAuthError, clearProfileSuccessful, showAuthError, UpdateUser } from '../../store/actions/authActions';
...
function EditProfileScreen() {
    ...
    const saveEditProfile = () => {
        uploadImageAsync().then((resp) => {
            ...
            dispatch(showAuthError('Ha ocurrido un error en el servidor. Por favor prueba en unos minutos.'))
        })
    }
    ...
    async function uploadImageAsync() {
        ...
        const blob: Blob = await new Promise((resolve, reject) => {
            ...
            if (editProfile.ProfileImage) xhr.open('GET', editProfile.ProfileImage.uri, true);
            xhr.send(null);
        });

        const ref = Firebase.storage().ref().child('imgProfile_' + editProfile.Email.trim())
    ...
    return (
        <View style={styles.container}>
            ...
            <ProfileForm user={editProfile} editUser={setEditProfile} saveUser={saveEditProfile} />
            ...
            {(authState.isUpdateProfileSuccessful) &&
                <Dialog message={'¡Enhorabuena! Se ha actualizado correctamente tu perfil.'}
                    buttonTitle={"Aceptar"}
                    pressButton={() => { dispatch(clearProfileSuccessful()), navigation.goBack()}}/>
            }
